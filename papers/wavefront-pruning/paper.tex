...
expectation. The expected maximum of the terminal cloud has an exact
layer-cake form whose gradient is a pivotal value, the expected
improvement from one more infinitesimal survivor, and for a fixed shadow
price on path-time the linearized best response is an obstacle problem
whose keep sets are upper tails: the \emph{wavefront policy}, which
retains a path while its propagated future pivotal value exceeds its
carrying cost. A fully corrective Frank--Wolfe method over wavefront
policies, with dual bisection on the price, solves the budget problem and
certifies its answer: the returned policy is feasible and its
suboptimality is bounded by the Frank--Wolfe gap plus a complementary
slackness term. In the committed example the adaptive policy improves the
expected maximum by 19\% over the best single screening date and by 30\%
over static thinning at the same expected budget. The finite-$n$ race with
a pathwise budget, and the continuous-time limit, are left to future work.
\end{abstract}
...
pays $1/\sqrt{2\pi} \approx 0.3989$ on the same expected budget. The
...
reaches an expected maximum of $1.9601$, against $1.6429$ for the best
one-shot screening policy and $1.5086$ for static random thinning. A
$100{,}000$-trial Monte Carlo puts the adaptive policy at $1.9609$ with
standard error $0.0021$. The analytic gradient of \eqref{eq:layercake}
